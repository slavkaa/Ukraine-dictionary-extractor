...
// @acton: php lcorp_move_results_to_word.php

require_once('../support/_require_once.php');

// *** //

$results = new LcorpResults($dbh);
...
for ($i = 0; $i < $counter;  $i++) {
    $result100 = new LcorpResults($dbh);
    ...
    foreach ($resultsPack as $resultsArr) {
        ...
        $wordText = array_get($resultsArr, 'accented_binary');
        $description = array_get($resultsArr, 'description');
        ...
        $value = (null == $value) ? '-' : $value;
        if (false == in_array($value, ['доконаний','недоконаний', 'доконаний і недоконаний', '-'])) {
        ...
        $value = (null == $value) ? '-' : $value;
        if (false == in_array($value, ['особовий','зворотній','взаємний','присвійний','вказівний','означальний','питальний','відносний','неозначений','заперечний', 'кількісний', '-'])) {
            var_dump($value);
            echo 'Wrong class';
            die;
        }
        ...
        $word = new LcorpWord($dbh);
        $word->firstOrNewTotal($wordText, $part_of_language, $creature, $genus, $number, $person, $kind, $verb_kind,
            $dievidmina, $class, $sub_role, $comparison, $tense, '-', $is_infinitive, $is_main_form, $variation, $main_form_code);

//        $word->updateProperty('html_id', PDO::PARAM_INT, $result->getId());

        if ($word->getId()) {
            $word->updateProperty('description', PDO::PARAM_STR, $description);
        }
