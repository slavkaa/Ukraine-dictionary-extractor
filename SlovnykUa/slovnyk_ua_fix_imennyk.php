...
// @acton: php slovnyk_ua_fix_imennyk.php
...
$sql = 'SELECT * FROM `html` WHERE part_of_language = \'іменник\' AND is_main_form = 1 and word_id is null ORDER BY id;';
$stm = $dbh->prepare($sql);
$stm->execute();

foreach ($stm as $htmlArray) {
    echo '+';
    $html = new Html($dbh);
    $html->getById(array_get($htmlArray, 'id'));

    echo ' HTML-ID ' . array_get($htmlArray, 'id') . '; ';
    ...
    foreach ($wordForms as $wordForm) {
        ...
        $htmlItem->firstOrNewNounNonUrl($word, $kind, $number, $dictionaryId);
